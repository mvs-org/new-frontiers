...
### Run a PoW/PoS Hybrid(Ethash & Grandpa) consensus test-net
...
```sh
./target/release/metaversevm --alice --validator \
...
```
./target/release/metaversevm --bob --validator \
  --base-path ./chaindata/node02 \
  --port 30334 \
  --ws-port 9946 \
  --rpc-port 9934 \
  --unsafe-rpc-external --rpc-methods Unsafe --rpc-cors all \
...
```
./target/release/metaversevm --charlie --validator \
  --base-path ./chaindata/node03 \
  --port 30335 \
  --ws-port 9947 \
  --rpc-port 9935 \
  --unsafe-rpc-external --rpc-methods Unsafe --rpc-cors all \
