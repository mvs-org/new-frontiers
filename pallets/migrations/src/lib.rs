...
pub trait Migration {
	...
	/// currently we don't have a  way to migate across blocks, so this method must (1) perform its full
	/// migration and (2) not produce a block that has gone over-weight. 
...
pub mod pallet {
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		/// on_runtime_upgrade is expected to be called exactly a single time after a runtime upgrade.
		...
		fn post_upgrade() -> Result<(), &'static str> {
			...
			// TODO: my wish to DRY all the things feels like this code is very repetitive...
